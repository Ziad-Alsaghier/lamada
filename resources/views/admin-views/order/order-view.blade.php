...
<div class="content container-fluid">
                    ...
                    <div class="row gy-2">
                        <div class="col-sm-6 d-flex flex-column justify-content-between">
                            <div>
                                    ...
                                    <div class="hs-unfold">
                                        <h5 class="text-capitalize">
                                            ...
                                            {{translate('table no')}} : <label
                                                class="badge badge-secondary">{{$order->table?$order->table->number:'Table
                                                deleted!'}}</label>
                                        </h5>
                                    </div>
                                ...
                                <div class="">
                                    {{translate('Order_Date_&_Time')}}: <i class="tio-date-range"></i>{{date('d M
                                    Y',strtotime($order['created_at']))}} {{ date(config('time_format'),
                        ...
                        <div class="col-sm-6">
                            <div class="text-sm-right">
                                <div class="d-flex flex-wrap gap-2 justify-content-sm-end">
                                    ...
                                    <div class="hs-unfold ml-1">
                                        ...
                                        <a class="btn btn-outline-primary" target="_blank"
                                            title="{{translate('Delivery Man Last Location')}}" data-toggle="tooltip"
                                            data-placement="top"
                                        ...
                                        <a class="btn btn-outline-dark" href="javascript:"
                                            onclick="last_location_view()" data-toggle="tooltip" data-placement="top"
                                ...
                                <div class="d-flex gap-3 justify-content-sm-end my-3">
                                    ...
                                    @else
                                    <span class="badge-soft-danger px-2 rounded text-capitalize">{{str_replace('_','
                                ...
                                @if(!in_array($order['payment_method'], ['cash_on_delivery', 'wallet_payment',
                                'offline_payment']))
                    ...
                    <table
                        ...
                        <tbody>
                            ...
                            <tr>
                                    ...
                                    <div class="media gap-3 w-max-content">
                                        ...
                                        <div class="media-body text-dark fz-12">
                                            ...
                                            <div class="d-flex gap-2">
                                                ...
                                                <span class="d-block text-capitalize">
                                                    {{ $value['label']}} :
                                                    <strong>{{\App\CentralLogics\Helpers::set_symbol(
                                                        $value['optionPrice'])}}</strong>
                                                </span>
                                                @endforeach
                                                @else
                                                @if (isset(json_decode($detail['variation'],true)[0]))
                                                <strong><u> {{ translate('Variation') }} : </u></strong>
                                                @foreach(json_decode($detail['variation'],true)[0] as $key1
                                                =>$variation)
                                                ...
                                                <span>
                                                    ...
                                                    @php($add_on_qtys==null? $add_on_qty=1 :
                                ...
                                <td class="text-right">{{\App\CentralLogics\Helpers::set_symbol($amount-$tot_discount +
                                    $product_tax)}}</td>
                            </tr>
                        ...
                        <div class="col-md-9 col-lg-8">
                            <dl class="row">
                                ...
                                <dd class="col-6 text-dark text-right">
                                    ...
                                    =$sub_total+$total_tax+$add_ons_cost-$total_dis_on_pro + $add_ons_tax_cost) }}</dd>
                                ...
                                <dd class="col-6 border-top pt-2 fz-16 font-weight-bold text-dark text-right">{{
                                    \App\CentralLogics\Helpers::set_symbol($sub_total - $order['coupon_discount_amount']
                                    - $order['extra_discount'] + $del_c) }}</dd>
                                ...
                                <dt class="col-6">
                                    <div class="d-flex max-w220 ml-auto">
                                        <span>
                                            {{translate('Paid By')}} ({{str_replace('_', '
                                            ',$partial->paid_with)}})</span>
                                        <span>:</span>
                                    </div>
                                </dt>
                                <dd class="col-6 text-dark text-right">
                ...
                <div class="card-body text-capitalize d-flex flex-column gap-4">
                    ...
                    <div class="card mt-3">
                        <div class="card-body text-center">
                            ...
                            <div class="mt-3">
                                <button class="btn btn-primary" type="button" data-id="{{ $order['id'] }}"
                                    data-target="#payment_verify_modal" data-toggle="modal">{{
                    ...
                    <div class="hs-unfold w-100">
                        ...
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu text-capitalize" aria-labelledby="dropdownMenuButton">
                                @if($order['payment_method'] == 'offline_payment' && $order->offline_payment?->status !=
                                1)
                                @if($order['order_type'] != 'dine_in')
                                <a class="dropdown-item" onclick="offline_payment_order_alert('{{ translate(" You can
                                    not change order status to this status. Please Check & Verify the payment
                                    information whether it is correct or not. You can only change order status to failed
                                    or cancel if payment is not verified.") }}')"
                                ...
                                <a class="dropdown-item"
                                    onclick="route_alert('{{route('admin.orders.status',['id'=>$order['id'],'order_status'=>'returned'])}}','{{ translate("
                                    Change status to returned ?") }}')" href="javascript:">{{translate('returned')}}</a>
                                <a class="dropdown-item"
                                    onclick="route_alert('{{route('admin.orders.status',['id'=>$order['id'],'order_status'=>'failed'])}}','{{ translate("
                                ...
                                @endif
                                <a class="dropdown-item"
                                    onclick="route_alert('{{route('admin.orders.status',['id'=>$order['id'],'order_status'=>'canceled'])}}','{{ translate("
                                    Change status to canceled ?") }}')" href="javascript:">{{translate('canceled')}}</a>
                                @else

                                @if($order['order_type'] != 'dine_in')
                                <a class="dropdown-item"
                                    onclick="route_alert('{{route('admin.orders.status',['id'=>$order['id'],'order_status'=>'pending'])}}','{{ translate("
                                    Change status to pending ?") }}')" href="javascript:">{{translate('pending')}}</a>
                                @endif

                                <a class="dropdown-item"
                                    onclick="route_alert('{{route('admin.orders.status',['id'=>$order['id'],'order_status'=>'confirmed'])}}','{{ translate("
                                    Change status to confirmed ?") }}')"
                                    href="javascript:">{{translate('confirmed')}}</a>

                                @if($order['order_type'] != 'dine_in')
                                <a class="dropdown-item"
                                    onclick="route_alert('{{route('admin.orders.status',['id'=>$order['id'],'order_status'=>'processing'])}}','{{ translate("
                                    Change status to processing ?") }}')"
                                    href="javascript:">{{translate('processing')}}</a>
                                <a class="dropdown-item"
                                    onclick="route_alert('{{route('admin.orders.status',['id'=>$order['id'],'order_status'=>'out_for_delivery'])}}','{{ translate("
                                    Change status to out for delivery ?") }}')"
                                    href="javascript:">{{translate('out_for_delivery')}}</a>
                                <a class="dropdown-item"
                                    onclick="route_alert('{{route('admin.orders.status',['id'=>$order['id'],'order_status'=>'delivered'])}}','{{ translate("
                                ...
                                @if($order['order_type'] == 'dine_in')
                                <a class="dropdown-item"
                                    onclick="route_alert('{{route('admin.orders.status',['id'=>$order['id'],'order_status'=>'cooking'])}}','{{ translate("
                                    Change status to cooking ?") }}')" href="javascript:">{{translate('cooking')}}</a>

                                <a class="dropdown-item"
                                    onclick="route_alert('{{route('admin.orders.status',['id'=>$order['id'],'order_status'=>'completed'])}}','{{ translate("
                    ...
                    {{-- <div class="">--}}
                        ...
                        {{-- <select name="order_status"
                            onchange="route_alert('{{route('admin.orders.status',['id'=>$order['id']])}}'+'&order_status='+ this.value,'{{translate("
                            Change the order status to ") }}'+  this.value.replace(/_/g, ' '))"
                            ...
                            {{-- <option value="out_for_delivery" {{$order['order_status']=='out_for_delivery'
                                ? 'selected' : '' }}>{{translate('Out_For_Delivery')}} </option>--}}
                            ...
                            {{-- @endif--}}
                            {{-- @if($order['order_type'] == 'dine_in')--}}
                            {{-- <option value="cooking" {{$order['order_status']=='cooking' ? 'selected' : '' }}>
                        ...
                        <div class="d-flex justify-content-between align-items-center gap-10 form-control">
                            ...
                            <label class="switcher payment-status-text">
                                <input class="switcher_input" type="checkbox" name="payment_status" value="1"
                                    id="payment_status_switch" onclick="offline_payment_status_alert('{{ translate(" You
                                    can not change status of unverified offline payment") }}')" {{$order->payment_status
                                == 'paid' ?'checked':''}}>
                    ...
                    <div class="">
                        {{--need change option--}}
                        <label class="font-weight-bold text-dark fz-14">{{translate('Delivery_Date_&_Time')}}
                            {{$order['delivery_date'] > \Carbon\Carbon::now()->format('Y-m-d')? translate('(Scheduled)')
                            : ''}}</label>
                        <div class="d-flex gap-2 flex-wrap flex-xxl-nowrap">
                            <input onchange="changeDeliveryTimeDate(this)" name="delivery_date" type="date"
                                class="form-control" value="{{$order['delivery_date'] ?? ''}}">
                            <input onchange="changeDeliveryTimeDate(this)" name="delivery_time" type="time"
                                class="form-control" value="{{$order['delivery_time'] ?? ''}}">
                        </div>
                    </div>
                    @if($order['order_type']!='take_away' && $order['order_type'] != 'pos' && $order['order_type'] !=
                    'dine_in' && !$order['delivery_man_id'])

                    <a href="#" class="btn btn-primary btn-block d-flex gap-1 justify-content-center align-items-center"
                    ...
                    <div class="">
                        ...
                        $order['order_status'] != CANCELED && $order['order_status'] != FAILED && $order['order_status']
                        != COMPLETED))
                        ...
                        <div class="form-control justify-content-between">
                            <span class="ml-2 ml-sm-3 ">
                                ...
                                <i class="tio-edit p-2 d-none" id="edit-icon" style="cursor: pointer;"
                                    data-toggle="modal" data-target="#counter-change" data-whatever="@mdo"></i>
                            </span>
                        </div>
                        @endif
                    </div>
                        ...
                        <div class="card-body">
                            ...
                            <div class="media flex-wrap gap-3">
                                ...
                                <div class="media-body d-flex flex-column gap-1">
                                    <a target="" href="#" class="text-dark"><span>{{$order->delivery_man['f_name'].'
                                            '.$order->delivery_man['l_name'] ?? ''}}</span></a>
                                    <span class="text-dark"> <span>{{$order->delivery_man['orders_count']}}</span>
                            ...
                            @else
                            <a href="javascript:" data-toggle="tooltip" class="text-dark" data-placement="top"
                            ...
                            @endif
                            @else
                            <a href="javascript:" onclick="last_location_view()" class="text-dark" data-toggle="tooltip"
                                data-placement="top"
                                title="{{translate('Only available when order is out for delivery!')}}">
                                <img width="13" src="{{asset('public/assets/admin/img/icons/location.png')}}"
                                    alt="">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; {{translate('Only available when order is out
                                for delivery!')}}
                            </a>
                            @endif
                        </div>
                            ...
                            {{--<div class="d-flex justify-content-between gap-3 border-top mt-3 pt-3">--}}
                                <div class="mb-4 d-flex gap-2 justify-content-between">
                                    ...
                                    <div class="edit-btn cursor-pointer" data-toggle="modal"
                                        ...
                                        {{-- <img width="24" src="{{asset('public/assets/admin/img/icons/edit.png')}}"
                                            alt=""> --}}
                                        <i class="tio-edit"></i>
                                    </div>
                                </div>
                                <div class="delivery--information-single flex-column">
                                    ...
                                    <div class="d-flex align-items-center gap-3">
                                        @if(isset($address['address']) && isset($address['latitude']) &&
                ...
                <div class="card mb-3">
                    <div class="card-body">
                        ...
                        <div class="media flex-wrap gap-3 align-items-center">
                            ...
                            <div class="media-body d-flex flex-column gap-1">
                                <a target="#" class="text-dark text-capitalize"><strong>{{translate('Guest
                                        Customer')}}</strong></a>
                            </div>
                        </div>
                        @else
                        ...
                        <div class="media flex-wrap gap-3">
                            ...
                            <div class="media-body d-flex flex-column gap-1">
                                <a target="_blank" href="{{route('admin.customer.view',[$order->customer['id']])}}"
                                    class="text-dark"><strong>{{$order->customer['f_name'].'
                                        '.$order->customer['l_name']}}</strong></a>
    ...
    <!-- Modal -->
    @if($order['order_type'] != 'pos' && $order['order_type'] != 'take_away' && ($order['order_status'] != DELIVERED &&
    $order['order_status'] != RETURNED && $order['order_status'] != CANCELED && $order['order_status'] != FAILED &&
    ...
    <div class="modal fade" id="counter-change" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog modal-sm" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel" style="font-size: 20px">{{ translate('Need time to
                        prepare the food') }}</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
            ...
            <div class="modal-content">
                ...
                <div class="card">
                    <div class="modal-body mx-2">
                        <p class="text-danger">{{translate('Please Check & Verify the payment information whether it is
                            correct or not before confirm the order.')}}</p>
                        <h5>{{translate('customer_Information')}}</h5>

                        <div class="card-body">
                            @if($order->is_guest == 0)
                            <p>{{ translate('name') }} : {{ $order->customer ? $order->customer->f_name.' '.
                                $order->customer->l_name: ''}} </p>
                ...
                <div class="btn--container justify-content-end my-2 mx-3">
                    ...
                    <a type="reset" class="btn btn-secondary" onclick="verify_offline_payment(2)">{{
                        translate('Payment_Did_Not_Received') }}</a>
                    @endif
                    <a type="submit" class="btn btn-primary" onclick="verify_offline_payment(1)">{{
                        translate('Yes,_Payment_Received') }}</a>
                </div>
            </div>
        </div>
    </div>
